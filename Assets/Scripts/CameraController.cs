...
public class CameraController : MonoBehaviour
{
    ...
    {
        float vertExtent = gameObject.GetComponent<Camera>().orthographicSize - 0.5f; //¬ертикальные границы камеры
        float horExtent = gameObject.GetComponent<Camera>().aspect * vertExtent - 0.5f; //√оризонтальные границы камеры
